...
int nAnonymizePlanetpayAmount = 1000;
...
#else
    const char* pszModule = "Planetpay";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Planetpay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Planetpay
    // Mac: ~/Library/Application Support/Planetpay
    // Unix: ~/.Planetpay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Planetpay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Planetpay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Planetpayd.pid"));
