...
void Shutdown()
{
    ...
    RenameThread("Planetpay-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Planetpay.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Planetpayd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePlanetpayamount=<n> " + _("Keep N Planetpay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Planetpay is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Planetpay is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Planetpay version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Planetpay server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Planetpay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Planetpay to complete") << "\n";
    ...
    nAnonymizePlanetpayAmount = GetArg("-anonymizePlanetpayamount", 0);
    if(nAnonymizePlanetpayAmount > 999999) nAnonymizePlanetpayAmount = 999999;
    if(nAnonymizePlanetpayAmount < 2) nAnonymizePlanetpayAmount = 2;
    ...
    LogPrintf("Anonymize Planetpay Amount %d\n", nAnonymizePlanetpayAmount);
