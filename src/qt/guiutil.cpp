...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Planetpay"))
        return false;
...
{
    // Convert Planetpay:// to Planetpay:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Planetpay://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Planetpay:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Planetpay.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Planetpay.lnk
...
{
    return GetAutostartDir() / "Planetpay.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Planetpay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Planetpay\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Planetpay app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Planetpay app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Planetpay-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Planetpay-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Planetpay-Qt"));
