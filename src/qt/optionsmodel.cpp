...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePlanetpayAmount"))
        settings.setValue("nAnonymizePlanetpayAmount", 1000);
    nAnonymizePlanetpayAmount = settings.value("nAnonymizePlanetpayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePlanetpayAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizePlanetpayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePlanetpayAmount:
            return QVariant(nAnonymizePlanetpayAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePlanetpayAmount:
            nAnonymizePlanetpayAmount = value.toInt();
            settings.setValue("nAnonymizePlanetpayAmount", nAnonymizePlanetpayAmount);
            emit AnonymizePlanetpayAmountChanged(nAnonymizePlanetpayAmount);
            break;
        default:
            break;
        }
