...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePlanetpayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePlanetpayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePlanetpayAmount = strAnonymizePlanetpayAmount.remove(strAnonymizePlanetpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlanetpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePlanetpayAmount*COIN) nMaxToAnonymize = nAnonymizePlanetpayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePlanetpayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePlanetpayAmount));
        strAnonymizePlanetpayAmount = strAnonymizePlanetpayAmount.remove(strAnonymizePlanetpayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlanetpayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePlanetpayAmount == 0){
